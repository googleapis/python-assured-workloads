...
def test_update_workload(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = assuredworkloads.Workload(
            ...
            il4_settings=assuredworkloads.Workload.IL4Settings(kms_settings=assuredworkloads.Workload.KMSSettings(next_rotation_time=timestamp_pb2.Timestamp(seconds=751))),
        )
...
def test_restrict_allowed_services(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.restrict_allowed_services),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_restrict_allowed_services_async(transport: str = 'grpc_asyncio', request_type=assuredworkloads.RestrictAllowedServicesRequest):
    client = AssuredWorkloadsServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(assuredworkloads.RestrictAllowedServicesResponse(
        ))
...
def test_analyze_workload_move(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.analyze_workload_move),
            '__call__') as call:
        # Designate an appropriate return value for the call.
